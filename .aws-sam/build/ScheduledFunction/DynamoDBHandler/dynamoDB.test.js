...
describe.only('DynamoDBHandler', () => {
    ...
    let sampleArticle;
    beforeAll(async () => {
        dbHandler = new dynamoDB_1.default(endpoint, accessKeyId, secretAccessKey);
        // Load the sample article from summarySample.json
        const sampleFilePath = path_1.default.resolve(__dirname, 'summarySample.json');
        const sampleData = fs_1.default.readFileSync(sampleFilePath, 'utf-8');
        sampleArticle = JSON.parse(sampleData);
        // Ensure tables exist
