...
describe('processRawMeetingData', () => {
  it('should process raw meeting data correctly', async () => {
    const charThreshold = 15000; // Example threshold

    const articles: Article[] = await processRawMeetingData({
        rawData: sample as RawMeetingData,
    ...
    expect(firstItem).toHaveProperty('imageKind');
    expect(firstItem).toHaveProperty('session');
    expect(firstItem).toHaveProperty('nameOfHouse');
    expect(firstItem).toHaveProperty('nameOfMeeting');
  }, 500000);
});